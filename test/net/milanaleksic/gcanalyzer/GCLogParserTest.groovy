...
class GCLogParserTest {
    ...
    @Test
    void parseSimpleGc() {
        GCEvents events = new GCLogParser().parse('2012-01-24T07:45:45.767+0000: 135213.292: [GC [PSYoungGen: 42939K->1191K(44480K)] 60259K->18511K(72256K), 0.0065950 secs] [Times: user=0.00 sys=0.00, real=0.01 secs] ')
    ...
    @Test
    void parseFullGc() {
        GCEvents events = new GCLogParser().parse('2012-01-24T07:43:16.086+0000: 135063.611: [Full GC (System) [PSYoungGen: 1105K->0K(44544K)] [ParOldGen: 17108K->17319K(27776K)] 18213K->17319K(72320K) [PSPermGen: 40705K->40646K(41088K)], 0.2675810 secs] [Times: user=0.31 sys=0.00, real=0.27 secs] ')
        assertThat(events.size(), equalTo(1))
        ...
        assertThat(event.stats['ParOldGen'].startValueInB, equalTo(1105L*1024))  // 17108K->17319K(27776K)
        assertThat(event.stats['ParOldGen'].endValueInB, equalTo(0L))
        assertThat(event.stats['ParOldGen'].maxValueInB, equalTo(44544L*1024))
        assertThat(event.stats['ParOldGen'].startValueInB, equalTo(1105L*1024))
        assertThat(event.stats['ParOldGen'].endValueInB, equalTo(0L))
        assertThat(event.stats['ParOldGen'].maxValueInB, equalTo(44544L*1024))
    }
